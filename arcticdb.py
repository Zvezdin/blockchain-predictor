...
def downloadBlockchain(start = 0, targetBlock = None):
	...
	while currentBlock < (targetBlock if targetBlock != None else 4146000): #todo determine the end of the blockchain automatically
		...
		saveData(chunkStore, blKey, blocks, blockChunkSize) #save block data
		if len(transactions) > 0 :
			saveData(chunkStore, txKey, transactions, txChunkSize) #save tx,t oo
...
def getLatestBlock():
	try:
		tmp = getLatestRow(chunkStore, blKey) #get a dataframe with only the latest row
